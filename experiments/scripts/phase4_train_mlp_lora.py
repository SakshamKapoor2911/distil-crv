...
def main():
    ...
    # 2. Wrap with LoRA (MLP Targeted)
    print("Wrapping with LoRAVerifier (Targeting MLP: gate_proj, up_proj, down_proj)...")
    ...
    print("Starting MLP LoRA Training...")
    verifier.train()
    ...
    res_path = os.path.join(results_dir, "mlp_lora_baseline.json")
    with open(res_path, "w") as f:
        json.dump(result_data, f, indent=2)
        
    # Save checkpoint
    torch.save(verifier.state_dict(), os.path.join(results_dir, "mlp_lora_verifier.pt"))
    print(f"\\nSaved results to {res_path} and model to mlp_lora_verifier.pt")
